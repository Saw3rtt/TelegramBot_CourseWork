    ...
    public static NotificationTask parseMessage(String messageText, Long chatId, TelegramBot telegramBot) {
        ...
        if (matcher.find() && matcher.groupCount() == 3) {
            ...
            try {
                ...
                telegramBot.execute(new SendMessage(chatId, "Ошибка"));
            }
        }
        telegramBot.execute(new SendMessage(chatId, "Ошибка. Не соответствует ожидаемому формату." +
                " Отправьте сообщение в следующем формате: dd.MM.yyyy HH:mm Текст напоминания." +
                " Время не должно быть в прошедшем времени"));
        return null;
    }
}